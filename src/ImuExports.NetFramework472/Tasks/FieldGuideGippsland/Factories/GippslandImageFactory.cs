...
namespace ImuExports.NetFramework472.Tasks.FieldGuideGippsland.Factories
{
    public class GippslandImageFactory : IFactory<GippslandImage>
    {
        public GippslandImage Make(Map map)
        {
            if (map != null &&
        ...
        public IEnumerable<GippslandImage> Make(IEnumerable<Map> maps)
        {
            var images = new List<GippslandImage>();
        ...
        private bool TrySaveImage(long irn)
        {
            try
            {
                using (var imuSession = ImuSessionProvider.CreateInstance("emultimedia"))
                {
                    ...
                    using (var file = File.Open($"{GlobalOptions.Options.Gip.Destination}{irn}.jpg", FileMode.Create, FileAccess.Write))
                    {
