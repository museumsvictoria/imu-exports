...
{
    public class GippslandAudioFactory : IFactory<GippslandAudio>
    {
        public GippslandAudio Make(Map map)
        {
            if (map != null &&
        ...
        public IEnumerable<GippslandAudio> Make(IEnumerable<Map> maps)
        {
            var audios = new List<GippslandAudio>();
        ...
        private bool TrySaveAudio(long irn, string filename)
        {
            try
            {
                using (var imuSession = ImuSessionProvider.CreateInstance("emultimedia"))
                {
                    ...
                    using (var file = File.OpenWrite($"{GlobalOptions.Options.Gip.Destination}{filename}"))
                    {
